...
import { convertDecimalTime, timeDiff } from "./sun-suntime/destime.js";
import suntime from "./sun-suntime/suntime.js";
...
namespace Thetkarit {
	// types
	export type YearView = YearViewObject;
	export type YearOptions = YearViewOptions;

	export type MonthView = MonthViewObject;
	export type MonthOptions = MonthViewOptions;

	export type DayView = DayViewObject;
	export type DayOptions = DayViewOptions;

	export type Languages = Language;
	export type ThinGyanData = ThinGyan;
	/**
	 * Time Zones
	 */
	export type TZ = TimeZones;
	export type CalConverterOptions = CalendarConvertOptions;
	/**
	 * Calendar Types
	 */
	export type CT = CalendarTypes;
	...
	export class Moon extends BcMoon {}
 ...
 */
	export class BurmeseCalendar {
		 ...
		 */
		public yearView({ year, lang = "English" }: YearOptions): YearView {
			this._year = year;
			this._lang = lang;
			return this.cal();
		}
		...
		public monthView({
			...
			month,
			lang = "English",
		}: MonthOptions): MonthView {
			this._year = year;
			this._lang = lang;
			return this.cal().month_views[month - 1] as MonthView;
		}
		...
		public dayView({
			...
			this._year = year;
			this._lang = lang;
			return (this.cal().month_views[month - 1] as MonthView).date_views[
				day - 1
			] as DayView;
		}
		/**
		...
		private thingyanDays(yearTo: number): ThinGyanData {
		...
		private thingyanFromG(gYear: number): ThinGyanData {
		 ...
		 */
		public burmeseYearType(by: number, lang?: Languages): string {
		 ...
		 * date.
		 */
		public calendarConverter({ ct, year, month, day }: CalConverterOptions): {
			year: number;
			month: number;
			day: number;
		} {
