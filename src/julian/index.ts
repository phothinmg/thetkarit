...
class BcGcal {
   ...
   */
  public jdUtcNow(): number {
    const utc = this.utcDtNow();
    return this.dt2jd({
      year: utc.year,
      month: utc.month + 1,
      day: utc.date,
  ...
  /**
   * Converts a Julian Date (jd) to a string representation of the datetime.
  ...
  public jd2dtStr(jd: number, tz: TimeZones = "GMT") {
    ...
    const g3 = this.jd2dt(jd, tz);
    const mo = ma[g3.month - 1];
    const h = g3.hour.toString().length === 1 ? `0${g3.hour}` : `${g3.hour}`;
    const mi =
      g3.minutes.toString().length === 1 ? `0${g3.minutes}` : `${g3.minutes}`;
    const s =
      g3.seconds.toString().length === 1 ? `0${g3.seconds}` : `${g3.seconds}`;
