...
import type { Bcal, JTB } from "./bcal/index.js";
import type { BcTranslate } from "./bcal/translate/index.js";
import type { Gcal } from "./gcal.js";
...
class Tcal {
	private _G: Gcal;
	private _B: Bcal;
	private _T: BcTranslate;
	private _lang: CTP.Language = "English";
	...
	private cal() {
		...
		const by_ids: number[] = [];
		// year object
		const year_object: CTP.YearViewObject = {
			year: {
				id: this._year,
				str: this._T.translateNum(this._year, this._lang),
				days_in_year: diy,
			},
			sasana_years: {
				ids: [],
		...
		for (let i = 0; i < 12; i++) {
			...
			const month_object: CTP.MonthViewObject = {
				year: {
					id: this._year,
					str: this._T.translateNum(this._year, this._lang),
				},
				month: {
					id: i + 1,
					long: this._T.translateStr(
						this._G.MONTHS[i] ?? "",
						this._lang,
					) as string,
					short: this._G.MONTH_SHORT[i] ?? "",
			...
			for (let j = 1; j <= days_InMonth; j++) {
				...
				const bcal: JTB = this._B.j2b(jdn);
				...
				bm_str.push(this._T.translateStr(bcal.bm_str, this._lang) as string);
				bm_index.push(bcal.bm);
				ssy_ids.push(bcal.ssy);
				by_ids.push(bcal.by);
				// _ssy
				_ssy = this._T.translateNum(bcal.ssy, this._lang);
				...
				// _by
				_by = this._T.translateNum(bcal.by, this._lang);
				if (typeof _by === "string") {
					by_str.push(_by);
				} else if (typeof _by === "number") {
					by_num.push(_by);
				}
				const date_object: CTP.DayViewObject = {
					jdn: jdn,
					year: {
						id: this._year,
						str: this._T.translateNum(this._year, this._lang),
					...
					day: {
						id: j,
						str: this._T.translateNum(j, this._lang),
					},
					weekday: {
						index: wd,
						long: this._T.translateStr(
							this._G.WEEK_DAYS[wd] ?? "",
							this._lang,
						) as string,
					...
					burmese_cal: {
						sasana_year: {
							id: bcal.ssy,
							str: this._T.translateNum(bcal.ssy, this._lang),
						},
						burmese_year: {
							id: bcal.by,
							str: this._T.translateNum(bcal.by, this._lang),
						},
						burmese_month: {
							index: bcal.bm,
							str: this._T.translateStr(bcal.bm_str, this._lang) as string,
						},
						moon_phase: {
							index: bcal.mp,
							str: this._T.translateStr(bcal.mp_str, this._lang) as string,
						},
						fortnight_day: {
							id: bcal.fd,
							str: this._T.translateNum(bcal.fd, this._lang),
						},
						burmese_day: {
							id: bcal.bd,
							str: this._T.translateNum(bcal.bd, this._lang),
						},
						sabbath: {
							index: astro.sabbath.index,
							str: this._T.translateStr(
								astro.sabbath.str,
								this._lang,
							) as string,
						...
						astro_days: this._T.translateStr(
							astro.astroDays,
							this._lang,
						) as string[],
						public_holiday: this._T.translateStr(
							hlds.holidaysArray,
							this._lang,
						) as string[],
					},
	...
	public yearView({
		year,
		lang = "English",
	}: CTP.YearViewOptions): CTP.YearViewObject {
		this._year = year;
		this._lang = lang;
		return this.cal();
	}
	...
	public monthView({
		...
		month,
		lang = "English",
	}: CTP.MonthViewOptions): CTP.MonthViewObject {
		this._year = year;
		this._lang = lang;
		return this.cal().month_views[month - 1] as CTP.MonthViewObject;
	}
	...
	public dayView({
		...
		day,
		lang = "English",
	}: CTP.DayViewOptions): CTP.DayViewObject {
		this._year = year;
		this._lang = lang;
		return (this.cal().month_views[month - 1] as CTP.MonthViewObject)
			.date_views[day - 1] as CTP.DayViewObject;
	}
	// --------------------------- THINGYAN -------------------------------------//
	/**
	...
	private thingyanTime(by: number): {
		...
		// Julian Date(jd) of Atat Time for given Burmese Year
		const ja: number = this._B.SY * by + this._B.MO;
		// Julian Day Number(jdn) of Atat Time for given Burmese Year
	...
	private thingyanDays(yearTo: number): CTP.ThinGyan {
		...
		const atat = this._G.jd2dt(tgTime.ja);
		...
		const akya = this._G.jd2dt(tgTime.jk);
		...
		const akyo = this._G.jd2dt(tgTime.dk - 1);
		...
		const akyat = this._G.jd2dt(tgTime.dk + 1);
		...
		if (tgTime.da - tgTime.dk > 2) {
			const akyat2 = this._G.jd2dt(tgTime.da - 1);
			AkyatDay2 = new Date(
				akyat2.year,
				akyat2.month - 1,
				akyat2.day,
		...
		const newyear = this._G.jd2dt(tgTime.da + 1);
	...
	private thingyanFromG(gYear: number): CTP.ThinGyan {
		const jdn = this._G.dt2jd({ year: gYear, month: 1, day: 1 }).jdn;
		const by = this._B.j2b(jdn).by;
		const yearTo = by + 1;
	 ...
	 */
	public burmeseYearType(by: number, lang?: CTP.Language): string {
		const lan = lang ?? "English";
		const yt = this._B.getLeapYearData(by).myt;
		const byt = this._B.YearTypes[yt];
		return this._T.translateStr(byt ?? "", lan) as string;
	}
	/**
	...
	public datetimeToJd({
		...
		return this._G.dt2jd({ year, month, day, hour, minutes, seconds, tz });
	}
	...
	public calendarConverter({
		...
		month,
		day,
	}: CTP.CalendarConvertOptions): {
		year: number;
		month: number;
		day: number;
	} {
		return this._G.calC({ ct, year, month, day });
	}
}
export default Tcal;
