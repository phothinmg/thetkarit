...
export class BcHolidays {
	...
	private sepHlds(jdn: number, hs: string[]) {
		if (special_holidays.includes(jdn)) {
			hs.push("Special Holiday");
		}
	}
	private sepSats(jdn: number, hs: string[]) {
		if (special_holidays.includes(jdn)) {
			hs.push("Substitute Working-day");
		}
	}
	private init() {
		const hhs: string[] = [];
